		...
		autocmds = {
			...
			lsp_codelens_refresh = {
				...
				-- list of auto commands to set
				{
					...
					-- the rest of the autocmd options (:h nvim_create_autocmd) desc = "Refresh codelens (buffer)",
					callback = function(args)
						if not require("astrolsp").config.features.codelens then
							return
						end
						if vim.lsp.codelens and vim.lsp.codelens.enable then
							pcall(vim.lsp.codelens.enable, true, { bufnr = args.buf })
						elseif vim.lsp.buf.code_lens then
							pcall(vim.lsp.buf.code_lens, args.buf)
						end
					end,
				},
			},
